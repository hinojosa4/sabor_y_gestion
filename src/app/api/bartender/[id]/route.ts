...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const { status, bartender_id } = body;
    ...
    if (bartender_id && !mongoose.Types.ObjectId.isValid(String(bartender_id))) {
      return NextResponse.json(
        { ok: false, message: "bartender_id no es un ID válido" },
        { status: 400 }
      );
    }
    ...
    if (status === "ready") {
      ...
      if (bartender_id) item.chef_id = String(bartender_id); // reutiliza campo chef_id
    }
    await item.save();
    ...
    return NextResponse.json(
      {
        ok: false,
        message: "Error al actualizar ítem de bartender",
