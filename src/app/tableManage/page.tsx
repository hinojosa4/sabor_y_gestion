...
export default function TableManagementPage() {
    ...
    const handleSubmitTable = async (tableData: any) => {
        try {
            let result;
            if (editingTable) {
                result = await updateTable(editingTable._id, tableData);
            } else {
                result = await addTable({
                    ...tableData,
                    restaurantId
                });
            }
    ...
    const OrderModal = ({ isOpen, onClose, table }: { isOpen: boolean; onClose: () => void; table: any }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50" style={{ zoom: 1.25 }}>
                        ...
                        <div className="p-4 md:p-6">
                            <div className="flex flex-col gap-6">
                                ...
                                <div className="relative w-full bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg border-2 border-gray-200 overflow-hidden" style={{ paddingBottom: '75%' }}>
                                    ...
                                    {/* Mesas dinámicas desde la base de datos */}
                                    {filteredTables.map((table) => {
                                ...
                                <div className="flex flex-wrap gap-4 justify-center text-sm">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-black">2 personas</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-8 h-8 rounded-full bg-green-500"></div>
                                        <span className="text-black">4 personas</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-10 h-10 rounded-full bg-green-500"></div>
                                        <span className="text-black">6 personas</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-12 h-12 rounded-full bg-green-500"></div>
                                        <span className="text-black">8+ personas</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            {/* Modal de orden (temporal - en desarrollo) */}
            <OrderModal
