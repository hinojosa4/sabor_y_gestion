  ...
  useEffect(() => {
    const calc = () => {
      ...
      if (diff < 60) setElapsed(`${diff}s`);
        ...
        setElapsed(
          `${Math.floor(diff / 3600)}h ${Math.floor((diff % 3600) / 60)}min`
        );
    };
    calc();
...
function TableCard({
  ...
  const isClickable = !["Reservada", "Inactiva", "Ocupada", "Cuenta solicitada"].includes(
    table.status
  );
    ...
    >
      ...
      <div style={{ padding: "12px 16px" }}>
        {order.items?.map((item) => (
          ...
          >
            <span>
              {item.quantity}x {item.dish_id?.name ?? "Plato"}
            </span>
            <span style={{ color: "#6b7280" }}>
              {formatBOB(item.unit_price * item.quantity)}
            </span>
          </div>
        ))}
      </div>
        ...
        <div style={{ padding: "0 16px 12px" }}>
          ...
          >
            ...
            {order.status === "pending"
              ? "Esperando que cocina tome la orden"
...
function OrderModal({
  ...
  const addToCart = (dish: Dish) =>
    setCart((prev) => {
      const ex = prev.find((i) => i.dish._id === dish._id);
      if (ex) return prev.map((i) => i.dish._id === dish._id ? { ...i, quantity: i.quantity + 1 } : i);
      return [...prev, { dish, quantity: 1, notes: "" }];
    });

  const removeFromCart = (dishId: string) =>
    setCart((prev) => {
      const ex = prev.find((i) => i.dish._id === dishId);
      if (!ex) return prev;
      if (ex.quantity <= 1) return prev.filter((i) => i.dish._id !== dishId);
      return prev.map((i) => i.dish._id === dishId ? { ...i, quantity: i.quantity - 1 } : i);
    });

  const getQty = (dishId: string) =>
    cart.find((i) => i.dish._id === dishId)?.quantity ?? 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "add" && existingOrderId) {
        ...
        res = await fetch(`/api/orders/${existingOrderId}/add-items`, {
          method: "PUT",
          headers,
          body: JSON.stringify({
            items: cart.map((i) => ({
        ...
        >
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#111" }}>
              {mode === "add"
                ? `Agregar a Orden — Mesa ${table.number}`
                : `Nueva Orden — Mesa ${table.number}`}
            </h2>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>
              {mode === "add"
                ? "Los nuevos platillos irán a cocina por separado"
        ...
        <div style={{ display: "flex", flex: 1, flexDirection: isMobile ? "column" : "row", overflow: "hidden" }}>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 20,
                    border: "1.5px solid",
                    borderColor: selectedCat === "all" ? "#111" : "#e5e7eb",
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 20,
                      border: "1.5px solid",
                      borderColor: selectedCat === cat._id ? "#111" : "#e5e7eb",
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p
                            style={{ margin: "4px 0 0", fontSize: 14, fontWeight: 700, color: "#ea580c" }}
                          >
                            {formatBOB(dish.price)}
                          </p>
                          {isBlocked && (
                            <p
                              style={{
              ...
              {isMobile && cart.length > 0 && (
                ...
                >
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#ea580c" }}>
                      {formatBOB(total)}
                    </p>
                  </div>
                  <button
          ...
          {(!isMobile || mobileTab === "cart") && (
            ...
            >
              ...
              <div style={{ padding: "16px 20px 8px" }}>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#111" }}>
                  {mode === "add" ? "Ítems a agregar" : "Orden Actual"}
                </p>
              </div>
                      ...
                      >
                        ...
                        >
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                              {formatBOB(item.dish.price * item.quantity)}
                            </p>
                          </div>
                          <button
                            onClick={() =>
                              setCart((prev) =>
                                prev.filter((i) => i.dish._id !== item.dish._id)
                              )
                            }
                            style={{
                              border: "none",
              ...
              <div style={{ padding: "16px 20px", borderTop: "1px solid #f3f4f6" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    background:
                      cart.length === 0 ? "#e5e7eb" : submitting ? "#94a3b8" : "#ea580c",
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("resize", check);
      ...
      const [tableOrderStates, setTableOrderStates] = useState<Record<string, TableOrderState>>({ });
        ...
        // ── Nuevo: tabla seleccionada para agregar a su orden ──────────
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      channel.bind("table:updated",        () => { if (mounted) fetchData(); });
  ...
  // ── Nuevo handler: abrir modal en modo "agregar" ───────────────
        ...
        <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "'Georgia', serif" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 8 : 14 }}>
              ...
              <div>
                <p style={{ margin: 0, fontSize: isMobile ? 15 : 17, fontWeight: 800, color: "#111" }}>
                  Panel de Mesero
                </p>
                {!isMobile && (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 8 : 16 }}>
              <div style={{ textAlign: "right" }}>
                <p style={{ margin: 0, fontSize: isMobile ? 12 : 13, fontWeight: 600, color: "#111" }}>
                  {user.name}
                </p>
                {!isMobile && <p style={{ margin: 0, fontSize: 11, color: "#9ca3af" }}>Mesero</p>}
              </div>
              <button
                onClick={() => logout()}
                style={{
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              >
                <h2 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 800, color: "#111" }}>
                  Mesas del Restaurante
                </h2>
                <p style={{ margin: "0 0 20px", fontSize: 13, color: "#9ca3af" }}>
                  Selecciona una mesa libre para tomar una orden
                </p>
                {loadingData ? (
                  <p style={{ color: "#9ca3af", fontSize: 13, textAlign: "center", padding: 32 }}>
                    Cargando mesas...
              ...
              >
                ...
                <p style={{ margin: "0 0 20px", fontSize: 13, color: "#9ca3af" }}>
                  Órdenes que tomaste en este turno
                </p>
            ...
            >
              ...
              >
                <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 800, color: "#111" }}>
                  Resumen del Turno
                </h3>
                {[
          ...
          {/* Modal agregar a orden existente */}
          {addToTable && (
            <OrderModal
              table={addToTable}
              mode="add"
              existingOrderId={tableOrderStates[addToTable._id]?.activeOrderId ?? undefined}
              onClose={() => setAddToTable(null)}
              onOrderCreated={() => {
                showToast("✓ Ítems agregados a la orden");
                fetchData();
              }}
            />
          )}
          ...
          {comandaOrderId && (
            <ComandaMeseroModal
              ...
              onClose={() => {
                setComandaOrderId(null);
                setComandaTableNumber(null);
              }}
            />
          )}
        </div>
