...
import { ComandaCocineroModal } from "@/components/comanda/ComandaModals";
...
// ─── Constantes ───────────────────────────────────────────────────────────────
const ALLOWED_ROLES: AuthUser["rol"][] = ["bartender", "admin"];

const STATUS_CONFIG = {
  pending:    { label: "Pendiente",      bg: "#fffbeb", border: "#fbbf24", accent: "#d97706", dot: "#f59e0b" },
  in_kitchen: { label: "Preparando",     bg: "#eef2ff", border: "#818cf8", accent: "#4338ca", dot: "#6366f1" },
...
// Paleta de bartender — azul-violeta (distinto al naranja de cocina)
const BRAND = {
  primary:   "#4f46e5",   // indigo
  secondary: "#7c3aed",   // violet
  accent:    "#06b6d4",   // cyan
  bg:        "#f5f3ff",   // lavender muy suave
  icon:      "🍹",
};
...
function getOrderNumber(order: BartenderOrder): string {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        gap: 8, borderBottom: "1.5px solid #f0f0f0",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 18 }}>{criticos.length > 0 ? "🚨" : "⚠️"}</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: criticos.length > 0 ? "#e85d26" : "#d97706" }}>
            Alerta de Inventario — Bartender
          </span>
        </div>
        <button onClick={onClose} aria-label="Cerrar alerta"
      ...
      <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8 }}>
        {criticos.map((a) => (
          <div key={a.ingredientId} style={{
            ...
            padding: "6px 10px", background: "#fff0ee", borderRadius: 8, border: "1px solid #fecaca",
          }}>
        ...
        {bajos.map((a) => (
          <div key={a.ingredientId} style={{
            display: "flex", justifyContent: "space-between", alignItems: "center",
            padding: "6px 10px", background: "#fffbeb", borderRadius: 8, border: "1px solid #fde68a",
          }}>
            <span style={{ fontSize: 13, fontWeight: 600 }}>{a.name}</span>
      ...
      <div style={{ padding: "8px 16px 14px", textAlign: "center" }}>
        <p style={{ margin: 0, fontSize: 11, color: "#aaa" }}>
          Stock descontado al iniciar preparación de bebidas
        </p>
      </div>
    </div>
  );
...
function LiveTimer({ createdAt }: { createdAt: string }) {
  ...
  return (
    <span style={{
      fontSize: 12, fontWeight: 700, color: URGENCY_COLORS[urgency].timer,
...
function StatusBadge({ status }: { status: OrderStatus }) {
  ...
  return (
    <span style={{
      ...
      background: cfg.bg, color: cfg.accent, border: `1.5px solid ${cfg.border}`,
      whiteSpace: "nowrap",
    }}>
      {cfg.label}
    </span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "space-between",
        gap: 8, borderBottom: `1.5px solid ${cfg.border}`, flexWrap: "wrap",
      }}>
                ...
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{
                      ...
                      {item.quantity}× {item.dish_id?.name ?? "Bebida"}
                    </p>
                    {catName && (
                      <p style={{ margin: "2px 0 0", fontSize: 11, color: BRAND.primary, fontWeight: 600 }}>
                        {BRAND.icon} {catName}
                      </p>
                    )}
                    {item.notes && (
      ...
      {order.status === "in_kitchen" && activeItems.length > 0 && (
        <div style={{ padding: "0 16px 8px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: "#94a3b8" }}>Bebidas listas</span>
      ...
      <div style={{ padding: "12px 16px", borderTop: "1.5px solid #f3f4f6" }}>
        {order.status === "pending" && (
          <button
            ...
            style={{
              ...
              background: loading ? "#94a3b8" : BRAND.primary,
              color: "#fff", fontSize: 13, fontWeight: 700,
              cursor: loading ? "not-allowed" : "pointer",
              fontFamily: "inherit", display: "flex", alignItems: "center",
              justifyContent: "center", gap: 8, transition: "background 0.15s", minHeight: 44,
            }}
          >
            {BRAND.icon} Preparar Bebidas
          </button>
        )}
        ...
        {order.status === "in_kitchen" && (
          ...
          >
            {allReady
              ...
              : `Faltan ${activeItems.length - readyItems.length} bebida(s)`}
          </button>
        )}
        ...
        {order.status === "ready" && (
          <div style={{
            ...
            ✓ Bebidas listas — esperando al mesero
          </div>
        )}

        <button
          onClick={() => onViewComanda(order._id)}
          style={{
            width: "100%", marginTop: 8, padding: "9px", borderRadius: 10,
            border: `1.5px solid ${BRAND.primary}`, background: "#eef2ff", color: BRAND.primary,
...
function KanbanColumn({
  col, onStatusChange, onItemToggle, actionLoading, onViewComanda,
}: {
  col: { key: string; title: string; orders: BartenderOrder[]; color: string; dot: string };
  onStatusChange: (orderId: string, status: OrderStatus) => Promise<void>;
  onItemToggle: (itemId: string, currentStatus: ItemStatus) => Promise<void>;
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "0 4px" }}>
        ...
        <h2 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#1a1a1a" }}>{col.title}</h2>
        <span style={{
          fontSize: 12, fontWeight: 700,
          background: col.color + "18", color: col.color,
          padding: "2px 10px", borderRadius: 20, border: `1px solid ${col.color}30`,
        }}>
          {col.orders.length}
        </span>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {col.orders.length === 0 ? (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: 13, color: "#cbd5e1" }}>Sin pedidos</p>
          </div>
        ) : (
...
function MobileKanban({
  columns, onStatusChange, onItemToggle, actionLoading, onViewComanda,
}: {
  columns: { key: string; title: string; orders: BartenderOrder[]; color: string; dot: string }[];
  onStatusChange: (orderId: string, status: OrderStatus) => Promise<void>;
  onItemToggle: (itemId: string, currentStatus: ItemStatus) => Promise<void>;
  actionLoading: boolean;
  onViewComanda: (orderId: string) => void;
  ...
  return (
    <div>
      <div style={{
        ...
        background: "#fff", borderRadius: 12, border: "1.5px solid #e2e8f0", overflow: "hidden",
      }}>
        {columns.map((c, i) => (
          <button
            key={c.key}
            ...
            style={{
              ...
              borderRight: i < columns.length - 1 ? "1.5px solid #e2e8f0" : "none",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: activeTab === i ? c.color : "#e2e8f0" }} />
              <span>{c.title}</span>
            </div>
            <span style={{ fontSize: 18, fontWeight: 800, color: activeTab === i ? c.color : "#94a3b8", lineHeight: 1 }}>
              {c.orders.length}
            </span>
          </button>
        ))}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {col.orders.length === 0 ? (
          <div style={{ textAlign: "center", padding: 48, background: "#fff", borderRadius: 12, border: "2px dashed #e2e8f0" }}>
            <p style={{ margin: 0, fontSize: 13, color: "#cbd5e1" }}>Sin pedidos</p>
          </div>
        ) : (
          col.orders.map((order) => (
...
export default function BartenderPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      console.error("Error al cargar órdenes de bartender:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      // Actualizar cuando llegan nuevas órdenes o cambian estados
      channel.bind("order:new",     () => fetchOrders());
      channel.bind("order:updated", () => fetchOrders());

      // Alertas de inventario — solo las del bartender (source: "bartender")
      channel.bind("inventory:alert", (data: { alertas: InventoryAlerta[]; source?: string }) => {
        if (data.alertas?.length > 0 && data.source === "bartender") {
          setInventoryAlertas(data.alertas);
          setTimeout(() => setInventoryAlertas(null), 12000);
        }
      });

      // Cuando delivery recoge/entrega, quitar la orden de la vista
  ...
  const handleStatusChange = async (orderId: string, newStatus: OrderStatus) => {
    ...
    try {
      ...
      const msgs: Record<string, string> = {
        in_kitchen: `${BRAND.icon} Preparando bebidas — inventario descontado`,
        ready:      "✓ Bebidas marcadas como listas",
        cancelled:  "Bebidas canceladas — stock restaurado",
      };
  ...
  const handleItemToggle = async (itemId: string, currentStatus: ItemStatus) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`/api/bartender/${itemId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: newStatus, bartender_id: user?._id }),
      });
      const data = await res.json();
      if (!data.ok) {
  ...
  const filterByCategory = (orderList: BartenderOrder[]) => {
  ...
  const columns = [
    { key: "pending",    title: "Pendientes",  orders: filterByCategory(pendingOrders),   color: "#d97706", dot: "#f59e0b" },
    { key: "in_kitchen", title: "Preparando",  orders: filterByCategory(inKitchenOrders), color: BRAND.primary, dot: "#6366f1" },
    { key: "ready",      title: "Listos",      orders: filterByCategory(readyOrders),     color: "#16a34a", dot: "#22c55e" },
  ];
  ...
  return (
    <div style={{ minHeight: "100vh", background: BRAND.bg, fontFamily: "'Georgia', serif" }}>

      {/* ── Header ── */}
      <div style={{
        background: "#fff", borderBottom: `2px solid ${BRAND.primary}`,
        ...
        <div style={{ display: "flex", gap: isMobile ? 6 : 12, alignItems: "center", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: isMobile ? 6 : 10 }}>
            {[
              ...
              { label: "Prep.",  value: inKitchenOrders.length, color: BRAND.primary },
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {!isMobile && (
              <div style={{ textAlign: "right" }}>
                ...
                <p style={{ margin: 0, fontSize: 10, color: BRAND.primary, fontWeight: 600 }}>Bartender</p>
              </div>
            )}
            <button
              onClick={() => logout()}
              style={{
                background: "#eef2ff", border: `1.5px solid ${BRAND.primary}`, borderRadius: 8,
                padding: isMobile ? "6px 10px" : "6px 14px",
                cursor: "pointer", fontSize: isMobile ? 11 : 12, fontWeight: 600,
                color: BRAND.primary, fontFamily: "inherit", whiteSpace: "nowrap", minHeight: 36,
              }}
            >
      ...
      {allCategories.length > 0 && (
        <div style={{
          paddingTop: "12px", paddingRight: px, paddingBottom: 4, paddingLeft: px,
          ...
          {["all", ...allCategories].map((cat) => (
            <button
              ...
              style={{
                ...
                borderColor: filterCat === cat ? BRAND.primary : "#e2e8f0",
                background: filterCat === cat ? BRAND.primary : "#fff",
                color: filterCat === cat ? "#fff" : "#64748b",
                fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                flexShrink: 0, whiteSpace: "nowrap", minHeight: 36,
              }}
            >
              {cat === "all" ? "Todas las bebidas" : cat}
            </button>
          ))}
          <button
            onClick={fetchOrders}
      ...
      <div style={{ padding: `16px ${px} 48px` }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: 80, color: "#94a3b8" }}>
            <p style={{ fontSize: 32, margin: "0 0 12px" }}>{BRAND.icon}</p>
            <p style={{ fontSize: 15 }}>Cargando bebidas...</p>
          </div>
        ) : orders.length === 0 ? (
          <div style={{
            ...
            <p style={{ fontSize: 48, margin: "0 0 12px" }}>{BRAND.icon}</p>
          ...
          <MobileKanban
            ...
            onViewComanda={setComandaOrderId}
          />
        ) : (
