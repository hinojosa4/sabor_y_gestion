...
interface OrderItem {
  ...
  dish: {  
    _id: string;
    name: string;
    category_id?: { nombre: string } | null;
  } | null;
}
...
// ─── Constantes ───────────────────────────────────────────────────────────────
const ALLOWED_ROLES = ["cocinero", "admin"] as const;
    ...
    <div style={{
      ...
      <div style={{ padding: "12px 16px", flex: 1 }}>
        {activeItems.map((item, idx) => {
          ...
          const catName = item.dish?.category_id?.nombre
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <p style={{
                    ...
                    {item.quantity}× {item.dish?.name ?? "Plato"}
                  </p>
                  {catName && (
                ...
                {/* Toggle item listo — solo en in_kitchen */}
      ...
      <div style={{ padding: "12px 16px", borderTop: "1.5px solid #f3f4f6" }}>
        ...
        {order.status === "in_kitchen" && (
          ...
          >
            {allReady ? "✓ Marcar como Listo" : `Faltan ${activeItems.length - readyItems.length} ítems`}
          </button>
        )}
...
export default function CocineroPage() {
  ...
  const { user, loading: userLoading, logout } = useAuth(
    ALLOWED_ROLES as unknown as import("@/lib/useAuth").AuthUser["rol"][]
  );
  ...
  const handleItemToggle = async (itemId: string, currentStatus: ItemStatus) => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`/api/orders/kitchen/item/${itemId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const allCategories = Array.from(
    new Set(
      orders.flatMap(o =>
        o.items
          .map(i => i.dish?.category_id?.nombre)
  ...
  const filterByCategory = (orderList: KitchenOrder[]) => {
    ...
    return orderList.filter(o =>
      o.items.some(i => i.dish?.category_id?.nombre === filterCat)
    );
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Georgia', serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 10 : 14 }}>
          ...
          <div>
            <p style={{ margin: 0, fontSize: isMobile ? 14 : 16, fontWeight: 700, color: "#1a1a1a" }}>
              Panel de Cocina
            </p>
            {!isMobile && (
        ...
        <div style={{ display: "flex", gap: isMobile ? 8 : 14, alignItems: "center" }}>
          {[
            ...
            <div key={s.label} style={{
              ...
              <p style={{ margin: 0, fontSize: isMobile ? 18 : 22, fontWeight: 800, color: s.color, lineHeight: 1 }}>{s.value}</p>
              <p style={{ margin: 0, fontSize: 9, color: "#94a3b8", fontWeight: 600, marginTop: 2 }}>{s.label}</p>
            </div>
          ))}
      ...
      {/* ── Contenido principal ── */}
      <div style={{ padding: `16px ${px} 40px` }}>
        {loading ? (
