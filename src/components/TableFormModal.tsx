...
export function TableFormModal({ isOpen, onClose, onSubmit, table, existingTables = [] }: TableFormModalProps) {
    ...
    const validateForm = (): boolean => {
        ...
        if (formData.seats < 1) {
            newErrors.seats = 'La capacidad debe ser al menos 1 persona';
        } else if (formData.seats > 20) {
            newErrors.seats = 'La capacidad no puede superar las 20 personas';
        }
    ...
    useEffect(() => {
        if (table) {
            setFormData({
                ...
                seats: table.capacity !== undefined ? table.capacity : (table.seats || 2),
                location: locationOptions.includes(table.location) ? table.location : '',
                customLocation: !locationOptions.includes(table.location) ? table.location : '',
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            min={1}
                            max={20}
                            className={`w-full min-w-0 rounded-md border px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 outline-none transition-all focus:ring-2 ${getFieldError('seats')
                                ? 'border-red-500 focus:border-red-500 focus:ring-red-500/30'
                                : 'border-gray-300 focus:border-gray-500 focus:ring-gray-500/30'
                                }`}
                            required
                        />
                        {getFieldError('seats') && (
                            <p className="text-red-500 text-xs mt-1">{getFieldError('seats')}</p>
                        )}
                    </div>
